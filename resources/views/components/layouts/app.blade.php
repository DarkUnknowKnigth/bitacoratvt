            ...
            <nav class="p-8 -mt-4 bg-blue-900 rounded-b-xl overflow-x-scroll overflow-y-hidden scrollbar-hidden-y">
                <ul class="flex flex-col gap-5 md:flex-row font-bold items-center justify-around text-center">
                    ...
                    @foreach ($modules as $module)
                        <li class="text-amber-500 text-center overflow-ellipsis dark:text-white hover:text-blue-800 hover:bg-amber-600 rounded-lg"><a class="px-2 py-2 text-center" href="{{ route($module->url) }}">{{$module->name}}</a></li>
                    @endforeach
                </ul>
            </nav>
            {{ $slot }}
