                ...
                <div class="flex flex-col items-center justify-center text-center">
                    <form wire:submit="{{$validation_id?'update('.$validation_id.')':'save()'}}" method="post">
                        <div class="md:grid md:grid-cols-4 flex flex-col gap-5 items-center justify-center w-full">
                            ...
                            <input type="text" name="value" wire:model="value" id="value" class="w-full md:w-auto rounded-lg px-3 py2 text-blue-950">
                            <label for="task">
                        ...
                        @if ($validation->tasks->count() > 0)
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-4 gap-2 space-y-2">
                                ...
                                <span class="flex flex-col gap-4 md:flex-row items-center justify-between p-2 bg-gray-50 dark:bg-slate-700 rounded-lg shadow-sm transition-transform transform hover:scale-[1.01] hover:shadow-md">
                                    ...
                                    <button class="px-3 py-2 bg-red-500 text-white rounded-lg flex flex-row gap-2"
                                        onclick="return confirm('¿Estás seguro de que quieres desvincular este elemento?') ? @this.call('unsync', {{ $validation->id }},{{ $task->id }}) : false;"
                                    >@include('icons.delete') Eliminar</button>
                                </span>
                                @endforeach
                            </div>
