        ...
        <div class="md:col-span-2 space-y-8">
            ...
            <div class="bg-white dark:bg-slate-800 rounded-xl shadow-lg py-6 flex flex-col items-center justify-center transition-all duration-300 transform hover:scale-[1.01]">
                ...
                <div class="flex flex-col items-center justify-center text-center">
                    <form action="" method="post" x-data="{main:1, name:'', parent:0}">
                        <div class="flex flex-col md:flex-row gap-5 items-center justify-center w-full">
                            ...
                            <select name="main" x-model="main" id="main"  class="w-full md:w-auto rounded-lg px-3 py2 text-blue-950">
                            ...
                            <select x-show="main==0" name="parent" x-model="parent" id="parent"  class="w-full md:w-auto rounded-lg px-3 py2 text-blue-950">
                                @foreach ($mainTasks as $t)
                                    <option value="{{ $t->id }}">{{$t->name}}</option>
                                @endforeach
                            </select>
                            <button type="submit" class="w-full text-white md:w-auto px-3 py-2 rounded-lg bg-amber-600 flex flex-row gap-2">@include('icons.save') Guardar</button>
                        </div>
                    </form>
                </div>
            </div>
                ...
                <ul class="space-y-4">
                    @forelse ($tasks as $task)
                        ...
                        @if ($task->validations->count() > 0)
                            <li class="gap-4 grid grid-cols-2 md:grid-cols-4 items-center justify-between p-4 bg-gray-50 dark:bg-slate-700 rounded-lg shadow-sm transition-transform transform hover:scale-[1.01] hover:shadow-md">
