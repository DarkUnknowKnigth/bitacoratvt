                        ...
                        <tbody>
                            @forelse ($locations as $location)
                                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                    <td class="px-6 py-4 font-medium text-gray-900 dark:text-white whitespace-nowrap">{{ $location->name }}</td>
                                    <td class="px-6 py-4">{{ $location->address }}</td>
                                    <td class="px-6 py-4 flex gap-2">
                                        <button class="gap-2 px-3 py-2 bg-yellow-400 text-gray-900 rounded-lg flex items-center justify-center" wire:click="edit({{ $location->id }}); document.getElementById('name').focus()">@include('icons.edit') Editar</button>
                                        <button class="gap-2 px-3 py-2 bg-red-500 text-white rounded-lg flex items-center justify-center" onclick="return confirm('¿Estás seguro de que quieres eliminar este elemento?') ? @this.call('destroy', {{ $location->id }}) : false;">@include('icons.delete') Eliminar</button>
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="3" class="text-center py-4 text-gray-500 dark:text-gray-400">No hay sucursales para mostrar.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
